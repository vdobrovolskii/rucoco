...
4. [Как отличается разметка конкретных и неконкретных упоминаний?](#как-отличается-разметка-конкретных-и-неконкретных-упоминаний)
...
3. На вопрос “как” короткого ответа нет 😊. Но описание будет состоять из нескольких пунктов:
   ...
   * Как отличается разметка конкретных и неконкретных упоминаний?
...
От ответов на эти вопросы зависит то, как мы будем размечать такое упоминания.

Давайте определимся с понятиями:
   * **Конкретное** упоминание — упоминание, отсылающее к одному _конкретному объекту_ или к _конкретному множеству_ таких объектов.
   * **Неконкретное** упоминание — любое другое упоминание.

### Конкретные/неконкретные объекты
...
Что **не** является конкретным объектом:
   ...
   * Нереферентные упоминания (такие, которые не отсылают к конкретной сущности, а называют класс объектов). Зачастую смысл высказывания не меняется, если добавить к этому упоминанию слова вроде "любой", "какой-либо", "никакой":
   ...
   * Моменты/отрезки времени: "вчера", "прошлые выходные", "2020 год", "полночь".
   * Параметры: "количество заболевших коронавирусом", "средний размер задолженности по ипотеке". Кстати, тест на участников сработает и здесь - "количество" и "размер" не существуют без участников.
...
"Окружающая среда" — это совокупность объектов живой и неживой природы, взаимодействующих с человеком. Таким образом, это множество определяется через квантор: "все объекты живой и неживой природы, взаимодействующие с человеком". Появление нового объекта, удовлетворяющего условию, автоматически делает его частью окружающей среды. Итого, "окружающая среда" — это _неконкретное_ множество. \
...
Несмотря на то, что количество здесь вполне определено, элементы множества по отдельности не являются конкретными объектами (момент/отрезки времени считаются неконкретными), поэтому это _неконкретное_ множество.
...
**Подытожим**: _конкретные_ упоминания -- это упоминания _конкретных объектов_ и _конкретных множеств_. Все остальные упоминания считаются _неконкретными_. Зачем нужно это различие? См. следующий раздел :)

## Как отличается разметка конкретных и неконкретных упоминаний?
Правила на самом деле довольно простые:
1. Для всех конкретных упоминаний мы размечаем **все** кореферентные упоминания в тексте.
2. Для неконкретных упоминаний мы размечаем только анафорические местоимения. При этом несколько цепочек не объединяются в одну:
...
**Внимание:** все в этом разделе относится только к _конкретным_ упоминаниям. Для неконкретных размечается только связь анафорического местоимения и его антецедента.
